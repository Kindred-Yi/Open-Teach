...
def main():
    ...
    with h5py.File(INPUT_FILE, 'r') as f:
        ...
        # 位置 (N, 3)
        pos_R = f['right_arm_cart']['positions'][:]
        pos_L = f['left_arm_cart']['positions'][:]
        
        # 方向 (N, 4)
        ori_R = f['right_arm_cart']['orientations'][:]
        ori_L = f['left_arm_cart']['orientations'][:]
        
        # Gripper (N, 12)
        joints_R = f['right_gripper']['positions'][:]
        joints_L = f['left_gripper']['positions'][:]
    ...
    X = np.hstack([
        feat_1, feat_2,          # 相对关系
        feat_3, feat_4, feat_7,  # 主手 (包含 Gripper)
        feat_5, feat_6, feat_8   # 副手 (包含 Gripper)
    ])
    ...
    with h5py.File(OUTPUT_FILE, 'w') as out:
        out.create_dataset('X', data=X_final, compression="gzip")
        out.create_dataset('Y', data=Y, dtype='i8', compression="gzip")
        
        # 记录每一列的含义
        col_desc = "1:Offset, 2:OffsetVel, 3:R_Trans, 4:R_Rot, 5:L_Trans, 6:L_Rot, 7:R_Grip, 8:L_Grip"
