...
def main():
    ...
    with h5py.File(INPUT_FILE, 'r') as f:
        ...
        # 位置 (N, 3)
        pos_R = f['right_gripper']['wrist_positions'][:3843]
        pos_L = f['left_gripper']['wrist_positions'][:3843]
        
        # 方向 (N, 4)
        ori_R = f['right_gripper']['wrist_orientations'][:3843]
        ori_L = f['left_gripper']['wrist_orientations'][:3843]
        
        # Gripper (N, 12)
        # joints_R = f['right_gripper']['actual_positions'][:]
        joints_L = f['left_gripper']['actual_positions'][:3843]
        joints_R = f['right_gripper']['commanded_positions'][:3843]
        # joints_L = f['left_gripper']['commanded_positions'][:]
        error_R = f['right_gripper']['error_positions'][:3843]
        error_L = f['left_gripper']['error_positions'][:3843]

        # 标签 (N, )
        labels = f['coordination_labels'][:3843]
