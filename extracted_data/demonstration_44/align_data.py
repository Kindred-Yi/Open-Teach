...
FILE_PATHS = {
    ...
    "left_arm_state": "left_franka_states.h5",
    "right_arm_state": "right_franka_states.h5",
    "left_gripper": "left tesollo_controller_state.h5",
    "right_gripper": "right tesollo_controller_state.h5"
}
...
def process_h5_file(h5_path, cam_times_sec, output_grp, prefix_name):
    ...
    with h5py.File(h5_path, 'r') as f:
        ...
        print(f"  - 读取到 {len(source_times)} 个时间戳")
        source_times_sec = to_seconds(source_times)

        # 检查数据有效性
        if len(source_times_sec) == 0:
            print(f"  错误: {prefix_name} 的时间戳数组为空，跳过。")
            return

        print(f"  - 时间范围: {source_times_sec[0]:.2f} ~ {source_times_sec[-1]:.2f} 秒")

        if len(cam_times_sec) == 0:
            print(f"  错误: 相机时间戳数组为空，跳过。")
            return
...
def main():
    try:
        ...
        print(f"相机时间戳加载成功，共 {len(cam_times_sec)} 个点")
        if len(cam_times_sec) > 0:
            print(f"  时间范围: {cam_times_sec[0]:.2f} ~ {cam_times_sec[-1]:.2f} 秒")
    except Exception as e:
        print(f"无法读取相机metadata: {e}")
        return

    if len(cam_times_sec) == 0:
        print("错误: 相机时间戳为空，无法继续处理。")
        return
    ...
    with h5py.File(OUTPUT_FILE, 'w') as out_f:
        ...
        process_h5_file(FILE_PATHS["left_arm_state"], cam_times_sec, out_f, "left_arm_state")
        process_h5_file(FILE_PATHS["right_arm_state"], cam_times_sec, out_f, "right_arm_state")
