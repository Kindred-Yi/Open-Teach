...
def main():
    ...
    with h5py.File(INPUT_FILE, 'r') as f:
        ...
        # 位置 (N, 3)
        pos_R = f['right_arm_state']['cartesian_positions'][:6300]
        pos_L = f['left_arm_state']['cartesian_positions'][:6300]
        
        # 方向 (N, 4)
        ori_R = f['right_arm_state']['cartesian_orientations'][:6300]
        ori_L = f['left_arm_state']['cartesian_orientations'][:6300]
        
        # Gripper (N, 12)
        joints_R = f['right_gripper']['actual_positions'][:6300]
        joints_L = f['left_gripper']['actual_positions'][:6300]
        # joints_R = f['right_gripper']['commanded_positions'][:]
        # joints_L = f['left_gripper']['commanded_positions'][:]
        error_R = f['right_gripper']['error_positions'][:6300]
        error_L = f['left_gripper']['error_positions'][:6300]
    ...
    # 拼接 X
    X = np.hstack([
        feat_9, feat_10, feat_3, feat_4, feat_5, feat_6          # 相对关系
    ])
